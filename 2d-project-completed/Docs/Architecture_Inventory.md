...
**상태:** **Phase 0~5·6·3(최소) 구현** — UI v2·`use_inventory_loadout` 시 활성 `weapon`→Player·HUD 세트 칩·W/RMB 스왑·가방 우클릭/더블클릭 장착. **미구현:** F5 기본 on, 퀵슬롯, offhand·방어구 스탯.

**목차:** [Overview](#overview) · [Responsibilities](#responsibilities--boundaries) · [Key Types](#key-types--relationships) · [Flow](#flow) · [UI 구성안](#ui-구성안) · [구현 단계](#구현-단계) · [Invariants](#invariants--gotchas) · [Change Guidelines](#change-guidelines)
...
데이터는 **카탈로그 정의(읽기 전용)** 와 **플레이어 상태(ID만 보관)** 를 분리한다. 세이브·UI·전투 적용은 모두 `item_id` 문자열을 경유하며, `WeaponData` 등 리소스는 런타임에 `ItemRegistry`로 해석한다.
...
| 스탯 합산 | 활성 세트 + (선택) 버프. 방어구·악세사리는 `Gun` 없이 스탯만 기여할 수 있음. |
...
ItemDefinition (Resource, 추상/공통)
  ...
  └─ stat_modifiers: Dictionary        # 예: {"armor": 12, "move_speed_mult": 1.05}

WeaponData extends ItemDefinition (또는 병행 + adapter)
  └─ 기존 weapon_type, hand, damage, gun 분기 필드 유지

GearData extends ItemDefinition        # helmet / armor / gloves / boots / accessory / shield
...
ItemRegistry (RefCounted / Autoload 후보)
  └─ resolve(item_id) -> ItemDefinition | null
  └─ resolve_weapon(item_id) -> WeaponData | null
...
InventoryLoadoutSeed (RefCounted)
  └─ F6 등 seed_demo_on_empty 시 데모 loadout 주입
...
1. **런 시작 / 로드** — `InventorySave.load_state()` → `InventoryService` 생성 시 `ItemRegistry.register_all_catalogs()`. F6 + `seed_demo_on_empty`면 빈 세이브에 `InventoryLoadoutSeed.apply_demo`.
2. **활성 세트 무기 → 전투** (`use_inventory_loadout == true`):
   ...
   - **미구현:** offhand `Gun`, 방어구 `stat_modifiers` → `player` 스탯(Phase 7).
...
2. 방어구·악세사리: `GearData` `.tres` 또는 장비 전용 카탈로그 GDScript.
3. `equip_slots` / `gear_slot` 잘못 지정 시 런타임 `can_equip`에서 거부 — 에디터 검증 스크립트는 선택.
...
`survivors_game.tscn` · `test_arena.tscn`에 `InventoryMenu` 인스턴스. F6는 `seed_demo_on_empty = true` 권장.
...
| 호버 | `detail` 패널 갱신 — 무기는 `build_select_tooltip_bbcode()`, 장비는 `stat_modifiers` 요약 |
...
- 무기 상세 BBCode를 인벤 스크립트에 **중복 작성**하지 말 것 — `WeaponData.build_select_tooltip_bbcode()` 재사용.
...
| **2** | 샘플 장비 카탈로그 | ✅ | `wooden_shield`·`leather_tunic`·`traveler_boots` |
...
| **산출물** | `inventory/equip_slots.gd`, `item_definition.gd`, `gear_data.gd`, `player_loadout_state.gd`, `inventory_save.gd`, `item_registry.gd`, `gear_catalog.gd` |
...
| 항목 | 내용 |
|------|------|
| **목표** | 무기 외 **offhand·방어구** 슬롯을 채울 테스트 데이터 |
| **산출물** | `inventory/data/*.tres` 2~3개 · `gear_catalog.gd` 등록 · (선택) `inventory/gear_catalog_builder.gd` |
| **예시 id** | `wooden_shield` (offhand), `leather_tunic` (armor), `traveler_boots` (boots) |
| **성공 기준** | `ItemRegistry.resolve_gear` · `can_item_occupy_slot(id, EquipSlots.OFFHAND)` true/false 기대대로 |
| **범위 밖** | 아이콘 아트 폴리시 |
| **의존** | Phase 0 |
...
| **산출물** | `player.gd` `refresh_stats_from_loadout` · offhand `Gun` 또는 패시브 · `WeaponPivot`/`OffhandPivot` 스프라이트 |
| **성공 기준** | 장비 `stat_modifiers`가 체력/이동 등에 반영 · 한손+방패 vs 양손 체감 차이 |
| **의존** | Phase 6 |
...
| 2 | `inventory/data/*.tres`, `inventory/gear_catalog.gd` |
...
| 7 | `player.gd`, `item_registry.gd` (`sum_stat_modifiers_for_set`), `entities/player/player.tscn` |
...
| 확인 항목 | 관련 |
|-----------|------|
| 새 장비 종류 | `GearData`(또는 `ItemDefinition`) + `ItemRegistry` 등록 + `equip_slot` |
...
| `inventory/gear_catalog.gd` | — | 샘플 `GearData` 3종 |
...
| `inventory/inventory_loadout_seed.gd` | `InventoryLoadoutSeed` | F6 데모 loadout (`preload` 호출 권장) |
| `ui/inventory/inventory_menu.gd` | — | UI v2 메뉴 |
...
인벤: **가방 8**, **2×7슬롯**, UI **무기·offhand 4칸**, `use_inventory_loadout` 시 활성 weapon→Player·HUD 세트 칩, **I**·**W**/닫힌 **RMB**·비활성 슬롯 **좌클릭** 전투 세트 전환, 가방 **우클릭/더블클릭** 장착.
