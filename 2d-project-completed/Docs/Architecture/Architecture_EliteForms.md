...
**구현 상태:** Phase 1 MVP-lite **대부분 구현** — affix `blazing`/`glacial`/`overloading` runtime, `PlayerDebuffController`, relic 드롭·가방·`RelicCombatBridge`, `KillRewards` affix XP ×1.5, F6 affix 드롭다운·mob affix 이름표 · **미구현:** `mending`/`gilded` affix behavior, `BalancePhase`↔roller `p_normal` 연동, affix HUD dot·SFX 폴리시 · F5 `affix_roll_enabled` 기본 `false` · **갱신:** 2026-06-03

affix(엘리트 형태) 롤·적용·런타임 행동, 플레이어 debuff, 유물(relic) 드랍·가방 보유 효과의 코드 경계와 훅 지점을 정리한다. affix별 수치·티어 1 스펙 본문은 Wiki에 두고, 이 문서는 **타입·흐름·불변조건**만 다룬다.
...
| affix 런타임 | 몹당 shield, bomb on-hit, death burst, ground hazard 등 (`mending`/`gilded`는 noop) |
| affix 이름표 | `mob.gd` — affix 표시명 Label (F6·전투 공통; HUD dot은 Wiki 후속) |
...
| `elite/affix/elite_affix_runtime_blazing.gd` | `blazing` — burn debuff, 잔불 hazard 스폰 |
| `elite/affix/elite_affix_runtime_glacial.gd` | `glacial` — chill on-hit, 사망 ice burst (`death_burst` 재사용) |
| `elite/affix/elite_affix_runtime_overloading.gd` | `overloading` — shield 50%·재충전, bomb debuff |
| `elite/affix/elite_affix_runtime_noop.gd` | `mending`/`gilded` 및 미등록 affix fallback |
| `elite/elite_ember_spawner.gd` | blazing 잔불 hazard 스폰 헬퍼 |
| `effects/elite_ember/elite_ember_hazard.gd` | blazing 지면 hazard tick |
| `elite/active_player_debuff.gd` | 활성 debuff 인스턴스 — duration·DoT·payload |
| `elite/player_debuff_data.gd` | 플레이어 debuff `Resource` 필드 |
| `elite/player_debuff_catalog.gd` | debuff id 등록 (`elite_burn`/`elite_bomb`/`elite_chill`/`elite_freeze`) |
| `elite/player_debuff_controller.gd` | `Player` 소유 — apply/tick/clear, heal·이동·stamina gate |
| `entities/mob/mob.gd` | `_elite_affix_id`, `elite_shield_hp`, affix 이름표, applier/runtime 위임, relic drop, `pool_reset` clear |
...
| `game/test_arena_mob_panel_controller.gd` | F6 Affix `OptionButton` — `(none)` / tier1 5종 (`mending` 포함) |
| `game/balance/balance_phase.gd` | (미연동) roller `p_normal` 입력 — 현재 `EliteAffixRoller` 하드코드 |
| `game/balance/kill_rewards.gd` | affix 보유 몹 XP ×1.5 (`has_elite_affix`) |
| `inventory/relic_data.gd` | `RelicData` — `held_effect` kind, on-hit/burst/heal 수치 |
| `inventory/relic_catalog.gd` | `relic_glacial`/`overloading`/`blazing`/`mending` 정의 |
| `inventory/relic_combat_bridge.gd` | 가방 relic dedupe → on-hit status / delayed burst / periodic heal |
| `inventory/item_registry.gd` | relic resolve, **장착 슬롯 거부** (`is_relic_item`) |
| `inventory/inventory_service.gd` | `acquire_relic()` — 가방만, auto-equip 없음 |
| `effects/equipment_drop/equipment_drop.gd` | relic 월드 드롭 (`RelicData` tint·표시명) |
| `effects/death_burst/death_burst_warning.gd` | glacial 사망 ice burst telegraph·AoE (AttackServices 경유) |
| `game/attack/damage_resolver.gd` | `apply_burst_damage_to_player_in_radius` — `elite_bomb` 만료 burst |
| `ui/player_status_panel.gd` | `elite_burn` HUD row (타 debuff dot은 Wiki 후속) |
...
Game.spawn_mob() / TestArena.spawn_test_mob()
  ...
  -> EliteAffixRuntime (blazing/glacial/overloading; mending/gilded noop)
...
2. `EliteAffixSpawnHelper.apply_after_mob_ready()`가 `EliteAffixRoller.roll(context)`를 호출한다. F5 기본값 `affix_roll_enabled = false` → **affix 0%** (F6는 `force_affix_id` 우선).
3. `EliteAffixRoller`가 `mob_kind`가 dummy/special_a/special_b면 **스킵**한다.
4. boss면 `p_boss`(v0.1: 100%)로 tier1 풀에서 1종 선택; 그 외는 phase별 `p_normal` Bernoulli 후 균등 가중. `p_normal` 수치는 roller 하드코드 — `BalancePhase` 연동은 **미연동**.
5. `EliteAffixApplier`가 HP·공격 export를 affix 배율로 재스케일하고, `slime_tint`·horn child(optional)를 붙인다. affix 표시명 Label을 동기화한다.
6. affix별 `EliteAffixRuntimeRegistry.create_runtime()` → `begin(mob)`. `blazing`/`glacial`/`overloading`은 전투 behavior; `mending`/`gilded`는 noop.
7. 매 physics tick에서 `Mob`이 runtime `tick(delta)`를 호출한다.
...
2. runtime `on_death()` — glacial ice bomb schedule 등 (affix별; `mending`/`gilded`는 noop).
...
**MVP-lite 최소 검증:** F6에서 affix `glacial`·`overloading`·`blazing` 강제 스폰 → E1~E3 → relic 치트 100% → E4~E5 → F5에서 `affix_roll_enabled=true` 후 9분+ affix 스폰 E6. 구현 Phase·잔여 항목은 [`BACKLOG.md`](../../BACKLOG.md) Elite Forms Epic을 따른다.
