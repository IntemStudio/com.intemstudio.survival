...
버프는 `buff/` 아래의 `BuffData` 정의와 `ActiveBuff` 런타임 상태로 나뉜다. `BuffController`는 대상별 활성 버프를 보관하고, 초·웨이브·충전 수 기반 만료를 처리한 뒤 `stat_modifiers`를 합산한다. 플레이어는 장비 스탯을 먼저 계산하고 활성 버프 modifier를 마지막에 곱해 이동속도, 무기 피해, APS에 반영한다. 현재 1차 구현은 플레이어 능력치 버프 중심이며, 몹의 poison/nettles 상태이상은 아직 `mob.gd` 기존 경로에 남아 있다.
...
| 플레이어 적용 | `Player`의 이동속도, 무기 피해, APS 계산에 활성 버프 modifier 반영 |
...
| `entities/player/player.gd` | `BuffController` 보유, 최종 이동속도·피해·APS 계산에 버프 반영 |
...
6. `BuffController` 변경 시 `Player._on_buffs_changed()`가 활성 modifier를 다시 계산하고 `Gun.refresh_loadout_combat_modifiers()`를 호출한다.
...
| 활성 버프는 원본 `WeaponData`, 기본 이동속도, 장비 modifier를 직접 수정하지 않는다. | 버프 만료와 세트 전환 시 원복 버그를 막는다. |
...
| 새 스탯 키 추가 | `GearStatMerge`, `LoadoutStatApply`, 장비/버프 툴팁 표시, 플레이어 적용 위치 |
