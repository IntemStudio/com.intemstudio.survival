...
F5 실행은 `game_lobby.tscn`에서 시작하고, 로비에서 서바이벌 또는 아레나를 선택해 `survivors_game.tscn`으로 전환한다. 선택한 모드는 `RunConfig`의 정적 상태로 전달되고, 실제 한 판의 오케스트레이션은 루트 `Game` 노드와 `game/game.gd`가 담당한다. 런은 시작 무기 획득으로 일시정지된 상태에서 시작하며, 첫 무기는 빈 활성 weapon 슬롯에 자동 장착된 뒤 선택한 모드의 스폰 규칙이 움직인다.
...
아레나 모드는 시간 강화 대신 `ArenaWaveDirector`가 1~10웨이브 큐를 관리한다. 웨이브마다 정해진 몹 목록을 Timer로 하나씩 스폰하고, 예정 스폰이 모두 끝난 뒤 살아 있는 몹이 0이 되면 웨이브 보상 무기 획득과 상자 구매를 연다. 아레나 몹 처치는 경험치 오브를 만들지 않으며, 다음 웨이브는 보상 단계 후 텔레포터 상호작용으로만 시작한다. 5/10웨이브는 보스 웨이브이며, 10웨이브를 모두 정리하면 기존 클리어 UI를 재사용한다.
...
| 책임 | 설명 |
|------|------|
| 런 시작 | 로비 이후 `survivors_game.tscn`, 시작 무기 획득, 첫 무기 자동 장착 후 스폰 타이머 시작 |
...
| 무기 성장 기획 | 무기 획득 UI 연결만 포함하고 성장 정책은 Wiki/Backlog에서 관리한다. |
| 출시 마일스톤 | `Plan_Release_Roadmap.md`에서 관리한다. |
| 맵 크기·소나무 튜닝 | `Docs/Agents/AGENTS_MapArena.md`가 작업 가이드를 가진다. |
| 영구 인벤토리 진행 | 런 한정 인벤토리 바깥의 메타 장비 진행은 별도 정책 결정 대상이다. |
...
| `game/game.gd` | 런 상태, 무기 획득, 스폰 타이머, 밸런스 tick, 종료 UI 오케스트레이션 |
...
| `ui/weapon_select_menu.gd` | 시작/레벨업 무기 획득으로 런 진행을 일시정지 |
...
GameLobby
  ...
  -> active weapon -> Player.add_weapon()
  -> Timer.timeout
...
4. 시작 무기 획득 UI가 열리며 트리는 pause된다. 후보가 없으면 즉시 런을 시작한다.
5. 무기 획득이 완료되면 런 인벤토리가 활성 세트의 빈 `weapon` 슬롯에 첫 무기를 자동 장착하고, 그 뒤 `_ensure_game_started()`가 선택한 모드의 스폰 규칙을 시작한다.
...
11. 아레나는 예정 스폰이 비고 살아 있는 몹이 0이면 현재 웨이브를 완료하고 무기 획득/상자 보상 단계를 연다. 보상 중에는 완료 판정을 보류한다.
12. 보상 단계가 끝나면 텔레포터를 다시 활성화한다. 다음 웨이브는 플레이어가 텔레포터에서 다시 `E`로 상호작용할 때만 시작된다.
...
| 규칙 | 이유 |
|------|------|
| 밸런스 시계는 `_game_started`이고 tree가 pause가 아닐 때만 증가한다. | 무기 획득, 일시정지, 게임오버 중 시간이 흐르지 않아야 한다. |
...
| 아레나 몹 처치는 경험치 오브를 드랍하지 않는다. | 무기 획득을 웨이브 클리어 보상으로 고정해 웨이브 진행 리듬을 유지한다. |
| 아레나 웨이브 보상 단계가 끝난 뒤에만 텔레포터를 다시 연다. | 보상 UI와 다음 웨이브 시작 입력이 겹치지 않게 한다. |
