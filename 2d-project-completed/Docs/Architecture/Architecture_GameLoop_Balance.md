...
아레나 모드는 시간 강화 대신 `ArenaWaveDirector`가 1~10웨이브 큐를 관리한다. 웨이브마다 정해진 몹 목록을 Timer로 하나씩 스폰하고, 예정 스폰이 모두 끝난 뒤 살아 있는 몹이 0이 되면 웨이브 보상 무기 획득과 선택형 상자 구매를 연다. 아레나 몹 처치는 경험치 오브를 만들지 않으며, 다음 웨이브는 텔레포터 상호작용으로만 시작한다. 5/10웨이브는 보스 웨이브이며, 10웨이브를 모두 정리하면 기존 클리어 UI를 재사용한다.
...
| 아레나 무기 획득 UI가 끝난 뒤에만 텔레포터를 다시 연다. | pause 보상 UI와 다음 웨이브 시작 입력이 겹치지 않게 한다. |
