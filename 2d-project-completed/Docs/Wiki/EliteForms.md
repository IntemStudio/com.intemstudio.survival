...
**상태:** 기획 v0.1 · 구현 **Phase 1~2 대부분** (affix `blazing`/`glacial`/`overloading`, 유물 4종, `PlayerDebuffController`, 드롭·가방·XP ×1.5) · **미구현:** `mending`/`gilded` affix behavior, horn mesh, affix HUD dot·드롭 VFX · **갱신:** 2026-06-03
...
현재 코드의 `mob_kind = elite` **독립 스폰 변종**과는 **다른 개념**입니다. affix 시스템으로 **통합·대체**하는 것이 목표이며, affix·유물 파이프라인은 **부분 구현** 중입니다.
...
| **데모 범위** | MVP-lite: affix 3종 + 유물 4종 **구현** · `mending`/`gilded` affix·`relic_gilded`·DLC는 **후속** |
...
| 항목 | 기획값 | 구현 |
|------|--------|------|
| 일반 몹 affix 롤 | **`p_normal`** — 아래 [등장 확률 기획안 (v0.1)](#등장-확률-기획안-v01) | roller 하드코드 · F5 `affix_roll_enabled` 기본 **off** |
| 보스 affix 롤 | **`p_boss = 100%`** — tier 1 affix 1종 (금빛 제외) | ✅ |
| affix 종류 가중치 | tier 1 네 종 **균등 25%** (금빛·공허 풀 제외) | ✅ (활성 풀 균등) |
...
F5 플레이테스트 전 **초안 수치**입니다. 구현: `EliteAffixRoller` 하드코드 · `BalancePhase` 연동은 **후속**.
...
> ⚠️ [`Items_Inventory.md`](Items_Inventory.md)의 “가방 = 효과 없음” 규칙과 **충돌**합니다. 유물만 예외 — **구현됨** (`RelicData`, `acquire_relic`, 장착 슬롯 거부).
...
| 대시 중 debuff tick | tick **정지**, 지속시간만 소모 (무적과 동일) · ✅ |
| tree pause | pause 중 tick **정지** · ✅ |
| “체력 재생 차단” (불타는) | 화상 중 `heal_health` **무효** + **스태미나 회복 배율 0** · ✅ |
...
| UI | 상태 |
|----|------|
| 몹 affix **이름표** (표시명 Label) | ✅ |
| 플레이어 **`elite_burn`** HUD row | ✅ |
| affix 색 dot · 이름 접두사 · 조준 UI | **후속** |
...
**상태:** ✅ affix·유물 `relic_blazing` · horn mesh **후속** (tint-only)
...
**상태:** ✅ affix·유물 `relic_overloading` · horn mesh **후속** (tint-only)
...
**상태:** ✅ affix·유물 `relic_glacial` · 왕관 mesh **후속** (tint-only)
...
**상태:** affix behavior **미구현** (noop) · 유물 `relic_mending` ✅ · horn mesh **후속**
...
| 오브젝트 | affix | 핵심 규칙 | 상태 |
|----------|-------|-----------|------|
| 잔불 지대 | 불타는 | 짧은 lifetime, 접촉 화상 | ✅ |
| 부착 폭탄 (debuff) | 과전하 | 플레이어 debuff, 1.5s 후 AoE | ✅ |
| 얼음 폭탄 | 빙하의 | 2s delay, AoE + 동결 | ✅ |
| 치유 코어 | 수리 중인 | 파괴 가능, neutral heal burst | **후속** |
| 금덩어리 | 금빛 | TTL 10s, 상호작용 획득 | **후속** |
...
| 현재 | affix 시스템 · 구현 |
|------|-------------------|
| `mob_elite.tscn` + `elite_spawn_ratio` | affix 롤로 **대체** — **후속** (병행 중) |
| affix 부여 몹 XP | **기본 kind XP × 1.5** · ✅ `KillRewards` |
| 몹 `status/` only | 플레이어 debuff **`PlayerDebuffController`** · ✅ |
| 가방 = 무효 | 유물만 **보유 적용** 예외 · ✅ |
| 몹 사망 드랍 | XP·골드·1% 픽업 + 유물 **0.025%** · ✅ |

### 구현 단계 (권장)

| 단계 | affix | 유물 | 인프라 |
|------|-------|------|--------|
| **1 MVP-lite** | `glacial`, `overloading` | `relic_glacial`, `relic_overloading` | ✅ affix 롤, F6, `PlayerDebuffController`, shield, death_burst |
| **2** | `blazing` ✅ · `mending` affix **후속** | `relic_blazing`, `relic_mending` ✅ | blazing hazard ✅ · heal aura·core **후속** |
| **3** | `gilded`, 공허 tier | `relic_gilded` | **후속** — DLC gate, `elite_spawn_ratio` 제거, pity·HUD dot |
...
| E5 | 동일 유물 2칸 | 효과 1스택만 |
| E6 | F5 9분+ (`affix_roll_enabled=true`) | `p_normal` affix 스폰 체감 |
...
- horn mesh (뿔·왕관·나선) — tint-only placeholder
- affix HUD dot · 이름 접두사 · 조준 UI · 드롭 VFX/SFX · minimap ping
- `BalancePhase` ↔ roller `p_normal` 연동
- 아레나 affix vs **엘리트 강화** modifier 역할 분리
- pity, 보스 1회 유물 보장, 튜토리얼 1회 고정 드랍
- 전용 relic 칸(가방 외) 여부
- **`mending` affix** — heal aura, healing core
- **`gilded` affix + `relic_gilded`** — DLC gate
- **공허 엘리트** tier 스펙·Void affix 풀
- `elite_spawn_ratio` / `mob_elite.tscn` 제거
- `p_normal` phase 보간·아레나 웨이브 테이블
- 유물 툴팁 BBcode
- [`Combat.md`](Combat.md) 플레이어 debuff 섹션 추가
...
| [`Items_Inventory.md`](Items_Inventory.md) | 장착 vs 가방 (유물 예외 · 구현됨) |
