...
**CLI smoke (`run_smoke.ps1`):** commit/PR·회귀용 게이트입니다. **사용자가 명시적으로 요청할 때만** Agent가 실행·계획에 포함합니다. (실행 must: [`.cursor/rules/godot-smoke-verify.mdc`](.cursor/rules/godot-smoke-verify.mdc))

F6/F5 수동 QA를 **대체하지 않습니다**. gdUnit4는 Godot `--headless` 직접 호출(디버거 hang 방지).

**실행 시 Agent 규칙:** `run_smoke.ps1`은 **한 번에 하나만** (foreground 권장). lock exit 2면 대기 또는 사용자에게 확인. `-AllowParallel` 사용 금지.
...
| `-UnitTestTimeoutSeconds` | gdUnit4 `test/` 타임아웃(초). 기본 `600`. `0`이면 무제한 |
| `-AllowParallel` | 단일 실행 lock 우회(디버그용). Agent/CI에서 사용 금지 |
