...
Godot 4.6 기반 2D 뱀파이어 서바이버류 프로젝트입니다. 한 판은 로비에서 시작해 무기 선택, 몹 스폰, 경험치 수집, 레벨업 선택, 패배 또는 30분 클리어로 끝납니다.
...
1. F5 로비에서 설정을 적용하고 게임 시작 시 `survivors_game.tscn`으로 이동합니다.
2. `Game._ready()`가 balance table/timeline을 준비하고 시작 무기 선택 UI를 엽니다.
3. 무기 선택 후 `Player.add_weapon()`이 호출되고 스폰 Timer가 시작됩니다.
4. Timer는 `Game.spawn_mob()`을 호출하고, `%MapArena` 위치 + `ScenePool`로 몹을 생성합니다.
5. `_process()`는 pause가 아닐 때 밸런스 시계, 타임라인 이벤트, 30분 클리어를 처리합니다.
6. 몹 사망은 `KillRewards`로 XP·골드를 계산하고 픽업을 생성합니다.
7. 레벨업은 무기 선택 UI를 열고, 사망/클리어는 게임오버 패널과 무기별 피해 목록을 표시합니다.
...
| 무기 선택 UI | `ui/weapon_select_menu.gd`, `survivors_game.tscn`, `Docs/Architecture/Architecture_Weapons.md` |
...
- F5 메인과 F6 테스트 아레나는 목적이 다릅니다. 테스트 편의를 위해 F5 기본 동작을 바꾸지 않습니다.
...
- 몹 일반 사망과 30분 클리어 사망을 섞지 않습니다. 클리어 사망은 드랍·처치 집계를 만들지 않습니다.
- 인벤토리 loadout은 F6 기본 on, F5 기본 off입니다. F5에서 켜기 전에는 레벨업 무기와 인벤 weapon 공존 정책을 먼저 확정합니다.
...
- F5: 로비 → 시작 무기 선택 → 스폰 → 레벨업 → 패배 또는 클리어 흐름 확인.
- F6: 테스트 아레나에서 몹 Spawn, 무기 Equip, 플레이어 리스폰 확인.
- UI 변경: 1280×720 기준 HUD·메뉴·게임오버·무기 선택 레이아웃 확인.
- 무기/몹 변경: 피해 통계, 풀 반환, 자동 공격 on/off, 원거리 예고/투사체 확인.
- 인벤 변경: `use_inventory_loadout == false`인 F5 회귀와 F6 loadout 반영 확인.
