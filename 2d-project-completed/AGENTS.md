...
Godot 4.6 기반 2D 뱀파이어 서바이버류 프로젝트입니다. 한 판은 로비에서 서바이벌 또는 아레나를 선택해 시작하고, 무기 선택, 몹 스폰, 경험치 수집, 레벨업 선택, 패배 또는 클리어로 끝납니다.
...
3. 시작 무기 선택 UI가 열리고, 무기 선택 후 `Player.add_weapon()`이 호출됩니다.
...
8. 레벨업은 무기 선택 UI를 열고, 사망/클리어는 게임오버 패널과 무기별 피해 목록을 표시합니다.
...
- F5 서바이벌: 로비 → 서바이벌 시작 → 시작 무기 선택 → 스폰 → 레벨업 → 패배 또는 30분 클리어 흐름 확인.
